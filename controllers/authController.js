  ...
  try {
    ...
    if (role === "user") {
      ...
      if (existingUser) {
        return res
          .status(400)
          .json({ error: "Email is already registered as a user." });
      }
      ...
      if (existingRestaurant) {
        return res
          .status(400)
          .json({ error: "Email is already registered as a restaurant." });
      }
      ...
      return res.status(400).json({
        error: 'Invalid role. Allowed values are "user" or "restaurant".',
      });
    }
  } catch (err) {
...
exports.login = async (req, res) => {
  ...
  try {
    if (!email || !password || !role) {
      return res
        .status(400)
        .json({ error: "Email/Phone, password, and role are required." });
    }
...
exports.forgetPassword = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred during forget password." });
  }
};
